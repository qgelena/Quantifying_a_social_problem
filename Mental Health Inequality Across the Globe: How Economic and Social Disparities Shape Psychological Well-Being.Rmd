...
merged_data <- merged_data %>%
  left_join(gdp_data, by = c("Entity" = "country", "Year" = "year"))

# merge corruption index
merged_data <- merged_data %>%
  inner_join(corruption_index, by = c("Entity" = "Country", "Year" = "Year"))
...
merged_data <- merged_data %>%
  left_join(urban_data, by = c("Entity" = "country", "Year" = "year"))
...
merged_data <- merged_data %>%
  inner_join(gov_spending_toGDP, by = c("Entity" = "Country", "Year" = "year"))
...
merged_data <- merged_data %>%
  left_join(gni_by_country_year,by = c("Entity" = "Country", "Year" = "year"))

GBD <- GBD %>%
  inner_join(gni_by_country_year, by = c("location" = "Country", "year_m" = "year"))

GBD <- GBD %>%
  inner_join(gini_cats, by = c("location" = "Entity", "year_m" = "Year"))
```

```{r summaries}
# Post-Merge Checks
# checking urban population variable 
...
merged_data <- merged_data %>% 
  rename(Depressive_disorders = `Depressive disorders`)
...
gg_miss_var(GBD, show_pct =  T)
```

```{r datasets summaries}
# short summaries of the final datasets
head(merged_data)
head(GBD)
```
...
merged_data[, c("gdp_per_capita", "income_group")]
```
...
GBD_dep_anx <- GBD[
  (GBD$cause %in% c("Anxiety disorders", "Depressive disorders")),
  ]

GBD_mental_disorders <- GBD[
  (GBD$cause %in% c("Mental disorders")),
  ]
...
GBD <- GBD %>%
  mutate(val_percent = val_m * 100)  # per 10,000

GBD_death <- GBD %>%
  filter(GBD$measure_name == "Deaths")

ggplot(GBD_death %>% filter(!is.na(val_m)), 
       aes(x = factor(year_m), y = val_percent, fill = cause)) +
  geom_col() +
  facet_wrap(~ cause, scales = "free_y", labeller = label_wrap_gen(25)) +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) +  
  labs(
    title = "Deaths Attributed to Mental Health Disorders Over Time",
    x = "Year",
    y = "Percent of Deaths per 10,000"
  ) +
...
GBD_Prevalence_depr <- GBD %>%
  filter(cause == "Depressive disorders", measure_name == "Prevalence", location %in% c("Ukraine", "Netherlands", "Germany", "Sweeden", "Greece"),!is.na(val_m)) %>%
  group_by(location, year_m)


ggplot(GBD_Prevalence_depr, aes(x = year_m, y = val_m, color = location)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
...
ggplot() +
  ...
  geom_sf(data = left_join(world, map_data, by = c("name" = "Entity")),
          aes(fill = Depressive_disorders)) +
...
GBD %>%
  filter(!is.na(income_group), measure_name == "Prevalence") %>%
  group_by(year_m, income_group) %>%
  summarise(mean_disorder = mean(val_m, na.rm = T)) %>%
  ggplot(aes(x = year_m, y = mean_disorder, color = income_group)) +
  geom_line(size = 1.2) +
  labs(
    title = "Disorder Rate Over Time by Income Group",
    x = "Year",
    y = "Mean Disorder Rate",
    color = "Income Group"
  ) +
  theme_minimal()
...
mental_summary <- mental_Sweden %>%
  ...
  group_by(year_m, cause) 

# Plot
p <- ggplot(mental_summary, aes(x = factor(year_m), y = val_m, fill = cause,
                               text = paste0("Year: ", year_m, "<br>",
                                             "Cause: ", cause, "<br>",
                                             "Avg Rate: ", comma(round(val_m, 2))))) +
...
ggplot(aes(x = income_group, y = Anxiety_disorders)) +
...
```{r Depressive disorder Rate by Gini Bucket}
# Shows the distribution (median, spread, outliers) of disorder rates for each gini group.

# Basic boxplot
merged_data %>%
  filter(!is.na(gini_bucket)) %>%
ggplot(aes(x = gini_bucket, y = Depressive_disorders)) +
  geom_boxplot(fill = "skyblue", alpha = 0.7) +
  labs(title = "Depressive disorder Rate by Gini Bucket",
       x = "Gini Coefficient Category",
       y = "Depressive disorders Rate (%)") +
  theme_minimal()
```

```{r Disorder by Gini Bucket Over Time}
GBD_dep_anx %>%
  filter(year_m %in% c(2014, 2020, 2021), measure_name == "Prevalence") %>%  # Limit to a few years for clarity
  ggplot(aes(x = gini_bucket, y = val_m)) +
  ...
  theme_minimal()
```
```{r skewness, vars picking}
#Check skewness and distribution for each numeric variable

colnames(merged_data)
# Pick numeric variables from dataset
vars_to_check <- c("gdp_per_capita", "Gini_index", "urban_pct", "pop_est", "housing_CosttoIncome",
                   "wealth_share_10",
                   "CPI_score", "PM2.5", "unemployment_rate", "gov_spending_toGDP", "Depressive_disorders", "Schizophrenia_disorders", "Anxiety_disorders",    "Bipolar_disorders", 
                   "Eating_disorders"
                   )
```
...
merged_data_log <- merged_data %>%
  mutate(
    log_eating = log(Eating_disorders +1),
...
merged_data_log %>%
  select(log_eating, log_gdp_per_capita, log_wealth_share_10,  log_unemployment_rate, log_population, log_PM2.5,
         Gini_index, Depressive_disorders, Anxiety_disorders, Schizophrenia_disorders, Bipolar_disorders, gov_spending_toGDP, housing_CosttoIncome) %>%
...
logs <- c("log_eating", "log_gdp_per_capita", "log_wealth_share_10", "log_unemployment_rate", "log_population", "log_PM2.5")
